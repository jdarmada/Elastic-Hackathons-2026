...
**The scheduled workflow:** the setup also creates an Elastic Workflow that runs **every day on its own** - it queries the latest results, next fixtures, and standout stories, has an LLM write a matchday briefing, and archives it in the `wc2026_daily_briefings` index. Demo it instantly from **Kibana → Workflows → WC2026 Daily Briefing → Run**, or ask the agent *"Give me today's briefing"* - it triggers the same workflow through the `generate_daily_briefing` tool. (If creation fails, check that Workflows is enabled in **Stack Management → Advanced Settings → Workflows** and re-run with `--agent-only`.)
